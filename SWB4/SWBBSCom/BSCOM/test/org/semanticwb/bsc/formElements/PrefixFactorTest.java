...
public class PrefixFactorTest {
    ...
    public void toRenderTest() {
        if (BSC.ClassMgr.hasBSC(modelId)) {
            ...
            Factor factor = Factor.ClassMgr.getFactor(factorId, model);//getRisk(riskId, model);
            if (factor != null) {
                System.out.println("Prefix Factor:" + factor.getPrefix());
            }
