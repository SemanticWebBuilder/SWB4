<%-- 
    Document   : msgReviewContainer
    Created on : 03-abril-2013, 17:50:37
...
//////////////////////
        ...
        url.setParameter("wsite", socialSite.getId());
...
out.println("  return '<img src=\"/swbadmin/css/images/feelneg.png\" /><br/><a href=\"javascript:void(0)\" onclick=\"showPopUp(\\'"+paramRequest.getRenderUrl().setMode(MsgReviewContainer.Mode_REVAL).setCallMethod(SWBResourceURL.Call_DIRECT)+"\\',\\'reevaluando\\',500,250)\" >revaluar</a>';");
out.println(" }else if(value==1) {");
out.println("  return '<img src=\"/swbadmin/css/images/feelpos.png\" /><br/><a href=\"javascript:void(0)\" onclick=\"showPopUp(\\'"+paramRequest.getRenderUrl().setMode(MsgReviewContainer.Mode_REVAL).setCallMethod(SWBResourceURL.Call_DIRECT)+"\\',\\'reevaluando\\',500,250)\" >revaluar</a>';");
out.println(" }else {");
out.println("  return '--<br/><a href=\"javascript:void(0)\" onclick=\"showPopUp(\\'"+paramRequest.getRenderUrl().setMode(MsgReviewContainer.Mode_REVAL).setCallMethod(SWBResourceURL.Call_DIRECT)+"\\',\\'reevaluando\\',500,250)\" >revaluar</a>';");
out.println(" }");
out.println("}");
...
out.println("}");
        ...
        out.println("   fillGrid(gridMaster, '"+url.setMode(MsgReviewContainer.Mode_JSON)+"');");
        out.println(" });");
        out.println("</script>");
...
if(paginas > 1) {
    ...
    String previusURL = "#";
    if (ipage < paginas) {
        nextURL = paramRequest.getRenderUrl().setParameter("ipage", Integer.toString(ipage+1)).setParameter("wsite", socialSite.getId()).setParameter("doView", "doView").toString();        
    }
    if (ipage > 1) {
        previusURL = paramRequest.getRenderUrl().setParameter("ipage", Integer.toString(ipage-1)).setParameter("wsite", socialSite.getId()).setParameter("doView", "doView").toString();
    }
    if (ipage > 1) {
    ...
    for (int i=ipage-2<=0?1:ipage-2; i<=ipage+2; i++)
    {
        if(i <= paginas){
            if(i==ipage)
                ...
                html.append("<a class=\"amfr-pagi\" href=\""+paramRequest.getRenderUrl().setMode(SWBResourceURL.Mode_VIEW).setParameter("ipage", Integer.toString(i)).setParameter("wsite", socialSite.getId()).setParameter("doView", "doView") +"\">");
            html.append(i);
            if(i==ipage)
                html.append("</strong>&nbsp;");
            else
