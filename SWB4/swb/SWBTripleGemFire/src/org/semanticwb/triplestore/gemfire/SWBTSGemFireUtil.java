 ...
 * @author jei
 */
public class SWBTSGemFireUtil
{
    private static Logger log = SWBUtils.getLogger(SWBTSGemFireUtil.class);
    ...
    public static String getHashText(String str)
    {
        ...
        if(str.length()>MAX_NODE_TEXT)
        {
            try
            {
                return SFBase64.encodeBytes(SWBTSGemFireChecksumCreator.getHash(str));
            }catch(Exception e)
            {
                log.error(e);
                return null;
