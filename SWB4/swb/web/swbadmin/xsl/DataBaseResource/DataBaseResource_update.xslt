...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/">
		<script src="/wbadmin/resources/DataBaseResource/js/CalendarPopup.js" ></script>
		<script >var calendar = new CalendarPopup();
				calendar.showYearNavigation();
				calendar.showYearNavigationInput();</script>
					...
					<xsl:for-each select="/FORMA/*">
						<xsl:if test="local-name()='SELECT'">
							if (document.forma.<xsl:value-of select="./@NOMBRE"/>
								<xsl:text>.value=="") {
    								alert("Elija una de las opciones de </xsl:text>
									<xsl:value-of select="./@ETIQUETA"/>
									<xsl:text>");
								document.forma.</xsl:text>
								<xsl:value-of select="./@NOMBRE"/><xsl:text>.focus();
    								return false;
  							}
							</xsl:text>
						</xsl:if>
	...
	<xsl:template match="TEXT">
		<tr>
			...
			<td >
				...
				<xsl:if test="./@TIPODATO='Date'"><xsl:text disable-output-escaping="yes">&lt;</xsl:text>a href="#" onClick="calendar.select(forma.<xsl:value-of select="./@NOMBRE"/>,'anchor','MM/dd/yyyy'); return false;" NAME="anchor" ID="anchor"<xsl:text disable-output-escaping="yes">&gt;</xsl:text>
