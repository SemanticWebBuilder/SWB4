 ...
 */
      ...
      function submitForm(formid)
      {
          ...
          if(!objd || objd.isValid())
          {
              try
              {
                ...
                //dojo.fx.wipeOut({node: formid, height: 100}).play();
              }catch(noe){}
      ...
      function addNewTab(id, url, title, tabName)
      {
          ...
          if(newTab==null)
          {
              ...
              //closeAll
              //alert(tabs.tablist.pane2button);
              //printObjProp(tabs.tablist.pane2button[newTab.id], true);
              //var menu=tabs.tablist.pane2menu[newTab];
              var menu=tabs.tablist.pane2button[newTab.id]._closeMenu;
      ...
      {
          removeTreeNodeByURI(uri, true);
      }

      function removeTreeNodeByURI(uri, closetab)
      {
          for(x=0;x<trees.length;x++)
          {
              var s=trees[x].model.store;
...
//          focusNodeByURI(uri);
      ...
      function addItem(store, item, parent)
      {
          if(getItem(store, item.id))return;
            ...
            //if(item.hasChilds)item.children=null;

            //alert("addItem2:"+store+" "+pInfo);
            var ite=store.newItem(item,pInfo);
            //alert("hasChilds1:"+item.hasChilds+" - "+item.children+" - "+item.children.length);

//            var childs=item.children;
//            alert((childs && childs.length>0)+" "+(item.hasChilds==true));
...
//            }
     ...
     function showStatus(msg, bgcolor)
     {
      ...
      {
          var button=dijit.byId(e.target.parentNode.id);
