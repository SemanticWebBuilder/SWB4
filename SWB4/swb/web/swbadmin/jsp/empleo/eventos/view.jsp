...
<%!
    ...
    public Set<Integer> getMonths(List<mx.gob.stps.portalempleo.swb.Event> eventos, int year)
    {
...
%><%
    ...
    if (months.size() > 0)
    {
        ...
        sdf = new SimpleDateFormat("dd 'de' MMMM 'de' yyyy", locale);
        
        SimpleDateFormat md = new SimpleDateFormat("MMM dd", locale);
        DateFormatSymbols simbols=md.getDateFormatSymbols();
        simbols.setShortMonths(new String[]{"Ene","Feb","Mar","Abr","May","Jun","Jul","Ago","Sep","Oct","Nov","Dic"});
        ...
        for (Event event : events)
        {
            ...
            if(event.getEnd()==null) {
                ...
                out.println("<span class=\"dia_calendario\">"+md.format(event.getStart())+" al "+  dd.format(event.getEnd()) +" </span>");
                //out.println("<span class=\"dia_calendario\">" + dd.format(event.getEnd()) + "</span>");
                
            }else if(event.getEnd()!=null && event.getStart().getMonth()!=event.getEnd().getMonth()){
            
                out.println("<span class=\"dia_calendario\">"+md.format(event.getStart())+" al "+  md.format(event.getEnd())  +"</span> a");
                //out.println("<span class=\"dia_calendario\">" + md.format(event.getEnd()) + "</span>");
            }
