...
<%!
    public List<Empresa> getSimilarEmpresas(Empresa e)
    {
        java.util.ArrayList<Empresa> empresas=new java.util.ArrayList<Empresa>();
        Iterator<Empresa> oempresas=Empresa.ClassMgr.listEmpresaByScian(e.getScian());
        while(oempresas.hasNext())
        {
            Empresa temp=oempresas.next();
            if(!temp.getURI().equals(e.getURI()))
            {
                empresas.add(temp);
            }
        }
...
<%
        Empresa oe = (Empresa)request.getAttribute("obj");
