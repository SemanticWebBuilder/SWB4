...
public class Test
{
    public static void main(String[] args) throws IOException
    {
        ...
        params.put("swb/mongodbname", "swb_test");
    ...
    //************* TEST  
            ...
            if (graph.count() < 100000)
            {
                try
                {
                    graph.load("/lenguaje/bibm/dataset_100M.nt", graph.count(), graph.count() + 100000);
                } catch (Exception e)
                {
                    e.printStackTrace();
                }
            ...
            //for (z = 0; z < 1000; z++)
            {
                TripleIterator it=graph.findTriples(new Triple("<http://www4.wiwiss.fu-berlin.de/bizer/bsbm/v01/instances/ProductType" + z + ">", null,null));            
//                x=it.count();
                while (it.hasNext())
                {
                    Triple triple = it.next();
