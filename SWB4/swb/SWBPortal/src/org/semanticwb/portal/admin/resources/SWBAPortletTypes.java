 ...
 */
public class SWBAPortletTypes extends GenericResource {

    private Logger log=SWBUtils.getLogger(SWBAPortletTypes.class);
    ...
    private String [] getCatSortArray(String [] strArray, String strOrder, int iWBPage, String tmid) {
        ...
        en1 = SWBContext.getWebSite(tmid).listPortletTypes();
        while (en1.hasNext()) {
            boolean iE=false;
            PortletType ROT = en1.next();
        ...
        while (it.hasNext()) {
            ...
            while (en.hasMoreElements()) {
                ...
                if (propResourceTypes.getProperty(element.toString()).equals(RO.getPortletClassName())) {
                    isExist=true;
                    break;
                }
            }
    ...
    private String [] getStrSortArray(String [] strArray, String strOrder, int iWBPage, String tmid) {
        Iterator<PortletType> en = SWBContext.getWebSite(tmid).listPortletTypes();
        int cont=0;
        if (strOrder.equals("tit")) {
            while (en.hasNext()) {
                PortletType RO = en.next();
            ...
            while (en.hasNext()) {
                PortletType RO = (PortletType)en.next();
                if (RO.getWebSite().getId().equals(tmid)) {
            ...
            while (en.hasNext()) {
                PortletType RO = (PortletType)en.next();
                if (RO.getWebSite().getId().equals(tmid)) {
                    strArray[cont]=RO.getPortletClassName()+":"+RO.getId();
                    cont++;
                }
            }
        }
            ...
            while (en.hasNext()) {
                PortletType RO = (PortletType)en.next();
                if (RO.getWebSite().getId().equals(tmid)) {
                    strArray[cont]=RO.getPortletMode()+":"+RO.getId();
                    cont++;
                }
            }
        }
    ...
    private Vector getAllReources (String strTmnid) {
        ...
        en1 = SWBContext.getWebSite(strTmnid).listPortletTypes();
        while (en1.hasNext()) {
            boolean iE=false;
            PortletType ROT = en1.next();
            for (int i=0; i<vRO.size();i++) {
    ...
    private int getArraySize(String strTmnid, boolean fromCat) {
        ...
        int cont=0;
        if (!fromCat) {
            Iterator<PortletType> en = SWBContext.getWebSite(strTmnid).listPortletTypes();
            while (en.hasNext()) {
                PortletType RO = en.next();
    ...
    private String getStrObjTable(PortletType RO, String bgcolor) {
        ...
        sbRet.append("</td> \n");
        sbRet.append("<td> \n");
        sbRet.append(RO.getPortletClassName());
        sbRet.append("</td> \n");
        sbRet.append("<td> \n");
        if(RO.getPortletMode()==1) sbRet.append(SWBUtils.TEXT.getLocaleString("com.infotec.wb.admin.resources.WBAResourceTypes","frmComboContent"));
        if(RO.getPortletMode()==2) sbRet.append(SWBUtils.TEXT.getLocaleString("com.infotec.wb.admin.resources.WBAResourceTypes","frmComboStrategy"));
        if(RO.getPortletMode()==3) sbRet.append(SWBUtils.TEXT.getLocaleString("com.infotec.wb.admin.resources.WBAResourceTypes","frmComboSystem"));
        if(RO.getPortletMode()==4) sbRet.append(SWBUtils.TEXT.getLocaleString("com.infotec.wb.admin.resources.WBAResourceTypes","frmComboInner"));
        if(RO.getPortletMode()==5) sbRet.append(SWBUtils.TEXT.getLocaleString("com.infotec.wb.admin.resources.WBAResourceTypes","frmPortletContent"));
        if(RO.getPortletMode()==6) sbRet.append(SWBUtils.TEXT.getLocaleString("com.infotec.wb.admin.resources.WBAResourceTypes","frmPortletStrategy"));
        sbRet.append("</td> \n");
        sbRet.append("</tr> \n");
    ...
    private String getIniForm(String strOrder, String strTm, String strTmSel, SWBParamRequest paramsRequest) throws SWBResourceException {
        ...
        if (strOrder==null || (strOrder!=null && (strOrder.equals("order") || strOrder.equals("map")))) {
            for (int i=iIniPage; i<iFinPage; i++) {
                ...
                PortletType RO = SWBContext.getWebSite(strTm).getPortletType(iArray[i]);
        ...
        if (strOrder!=null && (strOrder.equals("tit")||strOrder.equals("desc")||strOrder.equals("class")||strOrder.equals("type"))) {
            for (int i=iIniPage; i<iFinPage; i++) {
                ...
                if(!cambiaColor) rowColor="#FFFFFF";
                cambiaColor = !(cambiaColor);
                PortletType RO = SWBContext.getWebSite(strTm).getPortletType(Integer.toString(getObjId(strArray[i])));
                sbRet.append(getStrObjTable(RO,rowColor));
            }
        }
        sbRet.append("<tr  bgcolor=\"\" > \n");
...
//            }
    ...
    private String getEditForm(String strResId, WebPage topic, SWBParamRequest paramsRequest, String tm) throws SWBResourceException {
        ...
        if (iResId>0) {
            recObj=SWBContext.getWebSite(tm).getPortletType(Integer.toString(iResId));
            ...
            if (recObj.getPortletClassName()!=null)
                strClass=recObj.getPortletClassName();
            if (recObj.getPortletBundle()!=null)
                strBundle=recObj.getPortletBundle();
            if (recObj!=null) {
                iType=recObj.getPortletMode();
                iCache=recObj.getPortletCache();
            }
            //TODO: Portlet.getXML()
    ...
    public void processAction(HttpServletRequest request, SWBActionResponse response) throws SWBResourceException, IOException {
        ...
        if (act!=null && act.equals("add")) {
            try {
                ...
                PortletType pt = SWBContext.getWebSite(tm).createPortletType(ObjClass);
                pt.setCreator(user);
                pt.setTitle(ClassDisName);
                pt.setDescription(description);
                pt.setPortletMode(type);
                pt.setPortletClassName(ObjClass);
                pt.setPortletCache(cache);
                pt.setPortletBundle(BundleFile);
            ...
            try { //if(srvObj.UpdateObjResource(tm,iResId,ObjClass,ClassName,BundleFile,ClassDisName,description,type,cache,strXML.toString(),response.getUser().getId())) {
                ...
                PortletType pt = SWBContext.getWebSite(tm).getPortletType(ObjClass);
            ...
            try{ 
                ...
                SWBContext.getWebSite(tm).removePortletType(iResId);
