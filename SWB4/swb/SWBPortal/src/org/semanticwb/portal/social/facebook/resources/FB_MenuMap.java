 ...
 */
public class FB_MenuMap extends GenericAdmResource
{
    
    /** The log. */
    private static Logger log = SWBUtils.getLogger(FB_MenuMap.class);
    ...
    public void doView(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException
    {
        try
        {
            System.out.println("entra a FB_menuMap");
            HttpSession session = request.getSession(true);
            IFacebookRestClient<Document> userClient = getUserClient(session);
            if(userClient!=null){
                System.out.println("entra a FB_menuMap_1:"+userClient.getApiKey());
                Document dom=getDom(request, response, paramRequest);
                if(dom != null)  {
                    System.out.println("entra a FB_menuMap_2:"+dom);
