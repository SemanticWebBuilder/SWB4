...
public class GoogleGadget extends GenericResource
{
    ...
    public void processAction(HttpServletRequest request, SWBActionResponse response)throws SWBResourceException, IOException
    {
        ...
        log.debug("getQueryString en processAction: " + request.getQueryString());
        ...
        if ( url != null && !url.equals("") )
        {
            try
            {
                ...
                Iterator names = this.getResourceBase().getAttributeNames();
                while (names.hasNext())
                {
    ...
    public void doAddFromList2(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramsRequest) throws SWBResourceException, IOException
    {
        ...
        out.println("<td>B�scar gadgets en:<form action='" + urlChangeLang + "'><select onChange=\"javascript:this.form.submit();\" name='changelang'>");
    ...
    public void doAddFromList(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramsRequest) throws SWBResourceException, IOException
    {
        ...
        InputStream in = con.getInputStream();
        String html2 = SWBUtils.IO.readInputStream(in);
        log.debug(html2);
...
//        in.close();        
        ...
        if ( pos != -1 )
        {
            ...
            if ( urlGadget != null && !urlGadget.equals("") )
            {
                ...
                SWBResourceURL urlback = paramsRequest.getRenderUrl();
                urlback.setMode(urlback.Mode_ADMIN);
                out.println("<tr><td align='right' class='valores' ><form method='post' action='" + urlback + "'><input class='boton' type='submit' name='back' value='" + paramsRequest.getLocaleString("back") + "'></form></td></tr>");
    ...
    private Set<URL> getGadgets(String html)
    {
        ...
        while (pos != -1)
        {
            ...
            pos = html.indexOf("&url=http");
        }
        return getGadgets;
    }

    public void doChangeGadget(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramsRequest) throws SWBResourceException, IOException
    {
        PrintWriter out = response.getWriter();
        ...
        out.println("<table><tr><td class='datos'>URL del Gadget</td><td><input class='boton' type='text' name='url' size='50' maxlength='225'></td></tr>");
    ...
    public void doChangeConfig(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramsRequest) throws SWBResourceException, IOException
    {
        ...
        PrintWriter out = response.getWriter();
        out.println("<div class=box><form action='" + paramsRequest.getActionUrl() + "' method='post'>");
        out.println("<script type=\"text/javascript\" language=\"javascript\">");
        ...
        out.println("<tr><td class='datos'>" + paramsRequest.getLocaleString("titleOfGadget") + ":</td><td><input class='valores' type='text' size='50'  maxlength='100' name='title' value='" + title + "'></td></tr>");
        out.println("<tr><td class='datos'>" + paramsRequest.getLocaleString("width") + ":</td><td><input  class='valores' type='text' size='3' maxlength='3' name='w' value='" + w + "'></td></tr>");
        out.println("<tr><td class='datos'>" + paramsRequest.getLocaleString("height") + ":</td><td><input  class='valores' type='text' size='3' maxlength='3' name='h' value='" + h + "'></td></tr>");
        if ( url != null && !url.equals("") )
        {
            ...
            if ( languages.size() > 0 )
            {
                ...
                out.println("<tr><td class='datos'>" + paramsRequest.getLocaleString("language") + ":</td><td>");
            ...
            try
            {
                for ( String name : gadget.getParameterNames() )
                {
                    ...
                    {
                        out.println("<tr><td class='datos'>" + label + "</td><td>");
                        out.println("<input class='valores' type='text' size='10' name='" + name + "' value='" + defaultValue + "'>");
                        out.println("</td></tr>");
                    }
                    if ( dataType != null && gadget.isEnum(name) )
                    {
                        out.println("<tr><td class='datos'>" + label + "</td><td>");
                        out.println("<select class='valores' name='" + name + "'>");
        ...
        SWBResourceURL urlback = paramsRequest.getRenderUrl();
        urlback.setMode(urlback.Mode_ADMIN);
        out.println("<tr><td colspan='2' align='right'><input class='boton' type='button' Onclick='javascript:validData(this.form)' name='savechanges' value='" + paramsRequest.getLocaleString("changeConfiguration") + "'>&nbsp;&nbsp;&nbsp;<input class='boton' type='button' name='back' onClick=\" javascript:document.location='"+ urlback +"';\" value='" + paramsRequest.getLocaleString("back") + "'></form></td></tr>");
        out.println("</table>");
        out.println("</div>");
        out.close();
    }
    ...
    public void doAdmin(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramsRequest) throws SWBResourceException, IOException
    {
        ...
        if ( url != null && !url.equals("") )
        {
            ...
            try
            {
                ...
                out.println("<tr><td class='datos'>" + paramsRequest.getLocaleString("googleTitle") + ":</td><td class='datos'>" + title + "</td></tr>");
                out.println("<tr><td class='datos'><img width='120' height='60' src=\"" + image + "\"></td><td class='datos'>" + description + "</td></tr>");
            ...
            out.println("<tr><td class='datos'>" + paramsRequest.getLocaleString("orign") + ":</td><td class='datos'><a href='" + url + "' target='_blank'>" + url + "</a></td></tr>");
            if ( gadget.getAuthor() != null )
            {
                out.println("<tr><td class='datos'>" + paramsRequest.getLocaleString("author") + ":</td><td class='datos'>" + gadget.getAuthor() + "</td><td>");
            }

            if ( gadget.getAuthor_Location() != null )
            {
                out.println("<tr><td class='datos'>" + paramsRequest.getLocaleString("location") + ":</td><td class='datos'>" + gadget.getAuthor_Location() + "</td><td>");
            }
            ...
            out.println("<tr><td align='right'><form action='" + urladdFromList + "' method='post'><input class='boton'  type='submit' name='cambgad' value='" + paramsRequest.getLocaleString("addFromList") + "'></form></td><td align='right'><form action='" + urlchangeConfig + "' method='post'><input class='boton'  type='submit' name='cambgad' value='" + paramsRequest.getLocaleString("changeConfiguration") + "'></form></td></tr>");
            out.println("</table></div>");
            out.close();
        }
        if ( url == null || url.equals("") )
