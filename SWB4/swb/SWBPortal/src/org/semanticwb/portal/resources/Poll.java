...
public class Poll extends GenericResource {
    ...
    private boolean validateIPAddress(javax.servlet.http.HttpServletRequest request) {
        ...
        if (hashPrim != null && hashSec != null && hashPrim.size() > 0 && hashSec.size() > 0) { // Existe hash primario
            ...
            while (IHmP.hasNext()) {
                ...
                if (Hm != null && Hm == hashSec) {
                    ...
                    if (f1 != null && fctual.before(f1)) { // La fecha actual es menor que la que trae el hash primario, entonces se siguen
                        ...
                        if (!hashSec.containsKey(request.getRemoteAddr())) {
                            ...
                            hashPrim.clear();
                            if (hashPrim.isEmpty()) {
                                hashPrim.put(hashSec, f1);
                            }
                        } else {
                            flag = true;
                        }
                    } else if (f1 != null && (fctual.equals(f1) || fctual.after(f1))) { // La fecha actual es mayor a la que trae el hash primario, entonces 
        ...
        } else if (hashPrim != null && hashPrim.size() == 0) { // No existe hash primario. Aqu� entraria como primer paso.
            hashSec = new HashMap();
            hashSec.put(request.getRemoteAddr(), fctual);
            hashPrim = new HashMap();
            hashPrim.put(hashSec, Tfctualmoretime);
    ...
    private String getForm(javax.servlet.http.HttpServletRequest request, SWBParamRequest paramRequest) {
        ...
        try {
            ...
            ret.append("<table><tr>");
            ret.append("<td class=\"datos\">");
            ret.append(paramRequest.getLocaleString("actual_color"));
            ret.append("</td>");
            ret.append("<td width=\"20\"><table border=\"1\"><tr><td bgcolor=\"" + base.getAttribute("textcolor", "#000000") + "\" width=\"100%\">&nbsp;</td></tr></table>");
            ret.append("</td>");
            ret.append("</tr></table>");
            ret.append("</td>");
            ...
            ret.append(paramRequest.getLocaleString("actual_color"));
            ret.append("</td>");
            ret.append("<td><table border=\"1\"><tr><td bgcolor=\"" + base.getAttribute("textcolor", "#000000") + "\" width=\"10\"></td></tr></table>");
            ret.append("</td>");
            ret.append("</tr></table>");
            ret.append("</td>");
