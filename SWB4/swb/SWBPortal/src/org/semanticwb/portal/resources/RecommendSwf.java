...
/** Despliega y administra recomendaciones de usuarios finales interactuando 
 * con archivos de tipo Flash bajo ciertos criterios (configuración de recurso).
 * 
 *
 * Unfolds and administers recommendations of end users interacts with 
 * Flash type archives under certain criteria (resource configuration).
 *
...
public class RecommendSwf extends Recommend {
    ...
    public Document getDom(HttpServletRequest request,
        ...
        String action = (null != request.getParameter("rec_act") 
                ...
                ? request.getParameter("rec_act").trim() : "rec_step2";
        ...
        try {        
            ...
            if ("rec_step3".equals(action)) {
                ...
                if (!"".equals(base.getAttribute("swf", "").trim())) {
                    ...
                    el.setAttribute("swf", webWorkPath + "/" 
...
//                    }
                    strSwfvar += "&txtTopic=" + reqParams.getTopic().getTitle();
                    //strSwfvar += "&txtTopicDesc=" + reqParams.getTopic().getDescription(reqParams.getTopic().getMap().getTopic(reqParams.getTopic().getId() + user.getLanguage()));
                    strSwfvar += "&txtTopicDesc=" 
                            + reqParams.getTopic().getDescription(
                                                   user.getLanguage());
            ...
            log.error("Error while generating DOM in resource "
                    + base.getPortletType().getPortletClassName() + " with identifier "
    ...
    public void doAdmin(HttpServletRequest request, HttpServletResponse response,
        ...
        String action = (null != request.getParameter("act") 
                ...
                ? request.getParameter("act").trim() : paramsRequest.getAction();
        WBAdmResourceUtils wBAdmResourceUtils = new WBAdmResourceUtils();
        
        if (action.equals("add") || action.equals("edit")) {
            ret.append(getForm(request, paramsRequest));
        } else if(action.equals("update")) {
            FileUpload fup = new FileUpload();
            WBFileUpload wfu = new WBFileUpload();
            
            try {
                fup.getFiles(request, response);
                wfu.getFiles(request);
                String applet = null;
                String value = (null != fup.getFileName("template") 
                        && !"".equals(fup.getFileName("template").trim()))
                        ? fup.getFileName("template").trim() : null;
                if (value != null || !"".equals(base.getAttribute("template",
                                                                  "").trim())) {
                    if(value != null) {
                        //String file = WBResourceUtils.getInstance().getFileName(base, value);
                        String file = wBAdmResourceUtils.getFileName(base, value);
                        if (file != null && !file.trim().equals("")) {
                            if (!wBAdmResourceUtils.isFileType(file, "xsl|xslt")) {
                                msg = paramsRequest.getLocaleString(
                                        "msgErrFileType") + " <i>xsl, xslt</i>: "
                                        + file;
                            } else {
                                //applet = wBAdmResourceUtils.uploadFileParsed(base, fup, "template", request.getSession().getId());
                                applet = wBAdmResourceUtils.uploadFileParsed(
                                        base, wfu, "template", 
                                ...
                                if (applet != null && !applet.trim().equals("")) {
                                    ...
                                    msg = paramsRequest.getLocaleString(
                                            "msgErrUploadFile") +" <i>" + value
                                            + "</i>.";
                                }
                            }
                        } else {
                            msg = paramsRequest.getLocaleString(
                                    "msgErrUploadFile") +" <i>" + value + "</i>.";
                        }
                    }
                }
                value = (null != fup.getValue("noimg") 
                        && !"".equals(fup.getValue("noimg").trim()))
                        ? fup.getValue("noimg").trim() : "0";
                if ("1".equals(value) && !"".equals(base.getAttribute("img",
                        "").trim())) {
                    ...
                    value = (null != fup.getFileName("img")
                            ...
                            ? fup.getFileName("img").trim() : null;
                    if (value != null) {
                        String file = wBAdmResourceUtils.getFileName(
                                base, value);
                        if (file != null && !file.trim().equals("")) {
                            if (!wBAdmResourceUtils.isFileType(file, 
                                    "bmp|jpg|jpeg|gif")) {
                                msg = paramsRequest.getLocaleString(
                                        "msgErrFileType") 
                                        + " <i>bmp, jpg, jpeg, gif</i>: " + file;
                            } else {
                                if (wBAdmResourceUtils.uploadFile(base,
                                        fup, "img")) {
                ...
                value = (null != fup.getFileName("swf") 
                        ...
                        ? fup.getFileName("swf").trim() : null;
                if (value != null) {
                    String file = wBAdmResourceUtils.getFileName(base,
                            value);
                    if (file != null && !file.trim().equals("")) {
                        if (!wBAdmResourceUtils.isFileType(file,
                                "swf|emf|wmf")) {
                ...
                //base.updateAttributesToDB(paramsRequest.getUser().getId(), "Resource with identifier "+base.getId()+" was updated successfully ");
                ...
                //base.getRecResource().setXml(com.infotec.appfw.util.AFUtils.getInstance().DomtoXml(dom));
                base.setXml(SWBUtils.XML.domToXml(dom));
                // Ahora con la ejecución de la linea anterior no es necesaria la siguiente
            ...
            msg = wBAdmResourceUtils.removeResource(base);
            ret.append(
    ...
    private String getForm(HttpServletRequest request,
        ...
        try {
            ...
            //if (!"".equals(base.getAttribute("img", "").trim())) ret.append("<p>"+WBResourceUtils.getInstance().displayImage(base, base.getAttribute("img").trim(), "img") +"<input type=checkbox name=noimg value=1>" + paramsRequest.getLocaleString("msgCutImage") + " <i>" + base.getAttribute("img").trim() + "</i></p>");
            if (!"".equals(base.getAttribute("img", "").trim())) {
                ret.append("<p>");
                ret.append(wBAdmResourceUtils.displayImage(base, 
                        base.getAttribute("img").trim(), "img"));
                ret.append("<input type=checkbox name=noimg value=1>");
                ret.append(paramsRequest.getLocaleString("msgCutImage"));
                ret.append(" <i>" + base.getAttribute("img").trim());
                ret.append("</i></p>");
            }
            ...
            ret.append("</td> \n");
            ret.append("</tr> \n");
            ret.append(wBAdmResourceUtils.loadWindowConfiguration(base,
                    paramsRequest));
            ret.append("<tr> \n");
            ret.append("<td colspan=2>");
            ret.append("<br><br><font style=\"color: #428AD4; text-decoration: none; font-family: Verdana; font-size: 12px; font-weight: normal;\">");
    ...
    private String getScript(HttpServletRequest request,
        ...
        try {
            ...
            ret.append("\n   return true;");
            ret.append("\n}");
            ret.append(wBAdmResourceUtils.loadAddOption());            
            ret.append(wBAdmResourceUtils.loadEditOption());
            ret.append(wBAdmResourceUtils.loadUpdateOption());
            ret.append(wBAdmResourceUtils.loadDeleteOption());
            ret.append(wBAdmResourceUtils.loadDuplicateOption());
            ret.append(wBAdmResourceUtils.loadIsFileType());
            ret.append(wBAdmResourceUtils.loadReplaceChars());
            ret.append(wBAdmResourceUtils.loadIsNumber());
            ret.append("\n</script>");    
        } catch (Exception e) {
            log.error(e);
        }
