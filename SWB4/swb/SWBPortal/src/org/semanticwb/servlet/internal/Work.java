 ...
 * @author Jei
 */
public class Work implements InternalServlet
{
    
    /** The log. */
    static Logger log=SWBUtils.getLogger(Work.class);
...
//        }
        ...
        try
        {
            SWBUtils.IO.copyStream(new FileInputStream(file), out);
        }catch(Exception e)
        {
            log.error(e);
        }
     ...
     */
    protected boolean checkIfHeaders(HttpServletRequest request, HttpServletResponse response, File file)
        throws IOException
    {
     ...
     * @return the e tag
     */
    protected String getETag(File file)
    {
