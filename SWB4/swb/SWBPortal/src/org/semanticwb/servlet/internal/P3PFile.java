 ...
 * @author serch
 */
public class P3PFile implements InternalServlet {

    /** The log. */
    private static Logger log = SWBUtils.getLogger(P3PFile.class);
    ...
    {
        log.event("Initializing InternalServlet p3p.xml");
    }
    ...
    public void doProcess(HttpServletRequest req, HttpServletResponse resp, DistributorParams dparams) throws IOException, ServletException
    {
        ...
        String ret=SWBUtils.IO.getFileFromPath(SWBPortal.getWorkPath()+"/models/"+modelid+"/config/p3p.xml");
        if(ret==null)
        {
            ret=SWBUtils.IO.getFileFromPath(SWBPortal.getWorkPath()+"/config/p3p.xml");
        }
        if(ret!=null)
        {
