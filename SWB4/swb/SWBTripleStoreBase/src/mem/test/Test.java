...
import org.semanticwb.store.mem.GraphImp;
...
public class Test
{
    public static void main(String[] args) throws IOException
    {
        ...
        Graph graph = new GraphImp("swb",new HashMap());
    ...
    //************* TEST  
            ...
            if (graph.count() < 10000000)
            {
                try
                {
                    graph.load("/lenguaje/bibm/dataset_100M.nt", graph.count(), graph.count() + 1000000);
                } catch (Exception e)
                {
                    e.printStackTrace();
                }
