 ...
 * @author jei
 */
public class SWBTSMongoModelMakerExt
{
    private static Logger log=SWBUtils.getLogger(SWBTSMongoModelMakerExt.class);
    
    private HashMap<String,Integer> map;

    public SWBTSMongoModelMakerExt()
    {
        map=new HashMap();
    ...
    public Model getModel(String name)
    {
        ...
        if(id!=null)
        {
            return new ModelCom(new SWBTSMongoGraphExt(id,name));
        }else
        {
            return null;
        }
    ...
    public Model createModel(String name)
    {
        ...
        if(model==null)
        {
            try
            {
                ...
                index=new BasicDBObject("stype", -1);
                coll.createIndex(index);
                ...
                model=new ModelCom(new SWBTSMongoGraphExt(id,name));
                map.put(name, id);
    ...
    public void removeModel(String name)
    {
        ...
        if(model!=null)
        {
            int id=((SWBTSMongoGraphExt)model.getGraph()).getId();
            try
            {
                DB db = SWBTSMongoExt.getMongo().getDB(SWBPlatform.getEnv("swb/mongodbname","swb"));
                if(!db.isAuthenticated() && SWBPlatform.getEnv("swb/mongodbuser")!=null && SWBPlatform.getEnv("swb/mongodbpasswd")!=null)
