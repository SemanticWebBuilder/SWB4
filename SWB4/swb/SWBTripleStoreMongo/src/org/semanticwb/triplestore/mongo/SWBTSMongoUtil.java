 ...
 * @author jei
 */
public class SWBTSMongoUtil
{
    private static Logger log = SWBUtils.getLogger(SWBTSMongoUtil.class);
    ...
    public static String getHashText(String str)
    {
        ...
        if(str.length()>MAX_NODE_TEXT)
        {
            try
            {
                return SFBase64.encodeBytes(SWBTSMongoChecksumCreator.getHash(str));
            }catch(Exception e)
            {
                log.error(e);
                return null;
