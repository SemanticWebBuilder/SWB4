...
class Bar
{
    ...
    public boolean inSide(int ax, int ay)
    {
        ...
        if(p!=null)return p.contains(ax,ay);
...
public class WBGraph extends java.applet.Applet implements Runnable
{
    ...
    public void setZoomData(int x1, int x2)
    {
        ...
        }else
        {
            ...
            if(x1<sx)x1=sx;
            if(x2>(int)(Maxd*dr+sx-1))x2=(int)(Maxd*dr+sx-1);    
...
//        }
    ...
    {
        if(cgi==null)return super.getParameter(name);
	...
	if(type!=null)
        {
            if(type.equalsIgnoreCase("Bars"))_graphtype=BARS;
            if(type.equalsIgnoreCase("Lines"))_graphtype=LINES;
            if(type.equalsIgnoreCase("Pie"))_graphtype=PIE;
            if(type.equalsIgnoreCase("BarsPercent"))_graphtype=BARS_PERCENT;
            if(type.equalsIgnoreCase("Area"))_graphtype=AREA;
        }
        return _graphtype;
    }
    ...
    public void getParams(int x1, int x2)
    {
        ...
        if(x2>-1)ndata=x2-x1+1;                                  //zoom
        
        GifUrl+="&ndata="+ndata;
        int ncdata=Integer.parseInt(getParameter("ncdata"));
        GifUrl+="&ncdata="+ncdata;
        if(getParameter("percent")!=null && getParameter("percent").equals("true"))percent=true;
        if(getParameter("percent")!=null)GifUrl+="&percent="+getParameter("percent");
        if(getParameter("allpercent")!=null && getParameter("allpercent").equals("true"))allpercent=true;
        if(getParameter("allpercent")!=null)GifUrl+="&allpercent="+getParameter("allpercent");
        graphtype=getGraphType(getParameter("GraphType"));
        if(getParameter("GraphType")!=null)GifUrl+="&GraphType="+getParameter("GraphType");
        if(getParameter("BrakeLabels")!=null && getParameter("BrakeLabels").equals("false"))brakelabels=false;
        if(getParameter("BrakeLabels")!=null)GifUrl+="&BrakeLabels="+getParameter("BrakeLabels");
        if(getParameter("vzoom")!=null)vzoom=Boolean.valueOf(getParameter("vzoom")).booleanValue();
        if(getParameter("vzoom")!=null)GifUrl+="&vzoom="+getParameter("vzoom");
        if(getParameter("hvzoom")!=null)hvzoom=Boolean.valueOf(getParameter("hvzoom")).booleanValue();
        if(getParameter("hvzoom")!=null)GifUrl+="&hvzoom="+getParameter("hvzoom");
        if(getParameter("showgrid")!=null)showgrid=Boolean.valueOf(getParameter("showgrid")).booleanValue();
        if(getParameter("showgrid")!=null)GifUrl+="&showgrid="+getParameter("showgrid");
        if(getParameter("showpoints")!=null)showpoints=Boolean.valueOf(getParameter("showpoints")).booleanValue();
        if(getParameter("showpoints")!=null)GifUrl+="&showpoints="+getParameter("showpoints");
        if(getParameter("showseriesnames")!=null)showseriesnames=Boolean.valueOf(getParameter("showseriesnames")).booleanValue();
        if(getParameter("showseriesnames")!=null)GifUrl+="&showseriesnames="+getParameter("showseriesnames");
        if(getParameter("decimals")!=null)decimals=Boolean.valueOf(getParameter("decimals")).booleanValue();
        if(getParameter("decimals")!=null)GifUrl+="&decimals="+getParameter("decimals");
        
        if(decimals)formatterFloat = new DecimalFormat("0.00");
        ...
        for(x=0;x<ndata;x++)
        {
            ...
            {
                if(st.hasMoreTokens())data[x][y]=Double.parseDouble(st.nextToken());
                else data[x][y]=Double.NaN;
        ...
        for(x=0;x<ncolors;x++)
        {
            ...
            if(aux==null)aux=""+(int)(Math.random()*255)+","+(int)(Math.random()*255)+","+(int)(Math.random()*255);
        ...
        {
            int fsize=0;
            if(getParameter("Gif")!=null||getParameter("linkbk")!=null)fsize=35;
            if(showseriesnames)
            {
                for(x=0;x<ndata;x++)
                {
                    int ax=pad.getFontMetrics(f2).stringWidth(label[x]);
                    if(ax>fsize)fsize=ax;
                }
            }
        ...
        else
        {
            ...
            if(showseriesnames)
            {
                if(getParameter("zoom")!=null)fsize=35;
                for(x=0;x<ncdata;x++)
                {
        ...
        if(graphtype==BARS_PERCENT)md=90;         //Porcentage 100%
        if(hvzoom)
        {
        ...
        if(allpercent)sx+=10;
        if(percent)dr=((double)(Maxx-sx-40)/(double)Maxd); //diretancia entre barra
        else dr=((double)(Maxx-sx-20)/(double)Maxd);       //diretancia entre barra
        if(percent) mps=getMaxPorcentScale();  //max percent scale
        Title=getParameter("Title");
        target=getParameter("Target");
        if(getParameter("Title")!=null)GifUrl+="&Title="+getParameter("Title");
        SubTitle=getParameter("SubTitle");
        if(getParameter("SubTitle")!=null)GifUrl+="&SubTitle="+getParameter("SubTitle");
        if(getParameter("zoom")!=null)zoom=Boolean.valueOf(getParameter("zoom")).booleanValue();
    }
    ...
    public int getMaxPorcentScale()
    {
        ...
        for(x=0;x<Maxd;x++)
        {
            if(mx<data[x][Maxc-1])mx=data[x][Maxc-1];
        }
    ...
    public void renderPie()
    {
        ...
        if(Title!=null)pad.drawString(Title,Maxx/2-pad.getFontMetrics().stringWidth(Title)/2,9);
        pad.setFont(f2);
        pad.setColor(Color.red);
        if(SubTitle!=null)pad.drawString(SubTitle,Maxx/2-pad.getFontMetrics().stringWidth(SubTitle)/2,18);
        
        double tot=0;for(x=0;x<Maxd;x++)tot=tot+data[x][0];
        for(x=0;x<Maxd;x++)
        {
            bar[x][0]=new Bar();
            if(label[x].startsWith("_"))bar[x][0].parent=label[x].substring(1);
            else bar[x][0].parent=label[x];
    ...
    void Grabber()
    {
        ...
        for(y=0;y<cx;y++)
        {
            ...
            for(x=0;x<Maxd;x++)
            {
                ...
                int vx=(int)((double)data[x][0]*360/tot+0.5);
                pad.drawArc(cx,cy-y+cx,rx,ry,sx,vx);
                if(y>cx-2)pad.fillArc(cx,cy-y+cx,rx,ry,sx,vx);
                sx+=vx;
            }
        }
        pad.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
    ...
    private int leftLabel(int x)
    {
        ...
        if(label[x-1].startsWith("_"))
        {
            ...
            if(r<a)return (int)((dr*(x-1)+dr/2+sx)+l/2);
            else return r;
        }else
        {
            int l=getLabelWidth(x-1);
    ...
    private boolean canWriteLabel(int x)
    {
        ...
        if(le>leftLabel(x) && ri<rigthLabel(x))return true;
        return false;
    }
    ...
    public void render()
    {
        ...
        pad.setFont(f1);
        pad.setColor(Color.blue);
        if(Title!=null)pad.drawString(Title,Maxx/2-pad.getFontMetrics().stringWidth(Title)/2,12);
        pad.setFont(f2);
        pad.setColor(Color.red);
        if(SubTitle!=null)pad.drawString(SubTitle,Maxx/2-pad.getFontMetrics().stringWidth(SubTitle)/2,23);
        pad.setColor(Color.black);
...
//          else maxc=Maxc;
        ...
        {        
            ...
            if(percent && y==(maxc-1))graphtype=PERCENT;
...
//                }
    ...
    {//GEN-HEADEREND:event_formMouseDragged
        ...
        if(zoom)
        {
            ...
            if(x1<sx)x1=sx;
            if(x2>(Maxd*dr+sx-1))x2=(int)(Maxd*dr+sx-1);        
  ...
  private void formMouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_formMouseMoved
      ...
      double tot=0;for(int x=0;x<Maxd;x++)tot=tot+data[x][0];   //
      int sx=0;                                           //
...
//                  }
                  ...
                  if((evt.getX()+mxf+10)>getSize().getWidth())difx=-mxf-10;
                  if((evt.getX()+difx)<0)difx=difx-(evt.getX()+difx);
                  ...
                  repaint();
                  fl=true;
                  if(actbarname==bar[x][0].parent)enter=false;
      ...
      if(!enter && fl && !fl2 && !fl3)
      {
          ...
          if(getParameter("link")!=null)
          {
              papa.setCursor(12);
              try
              {dir = new URL(getDocumentBase(),getParameter("link")+actbarname);}catch (MalformedURLException e)
              {}
          }
  ...
  private void mouseClick(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_mouseClick
      ...
      }else
      {
          if(enter && dir!=null)
          {
              ...
              if(target==null)apc.showDocument(dir);
              else apc.showDocument(dir,target);
          }
      }
  ...
  public String formatNum(String num)
  {
      ...
      for(int x=0;x<l;x++)
      {
          if((l-x)%3==0 && (l-x)!=l)aux=aux+","+num.substring(x,x+1);else aux=aux+num.substring(x,x+1);
      }
      aux=ax+aux+dec;
      return aux;
  }
