...
import com.infotec.appfw.exception.AFException; //wb.core.exception.WBException;
import com.infotec.wb.util.*;
import com.infotec.appfw.util.*;
//import com.infotec.wb.core.indexer.WBIndexMgr;
//import com.infotec.wb.core.indexer.WBIndexer;
import com.infotec.wb.resources.*;
 ...
 * para su revisi�n � modificaci�n, subir archivos actualizados, eliminarlos
 * del repositorio, teniendo la posibilidad de recuperarlos a trav�s del
 ...
 * for a document revision or document modification, upload updated document, erase
 * from the repository, and have the posibility to restore eraded documents via an
 * administrator user. Also have a document preview. In the administration side,
 * the administrator define the user roles to perform a view, modify or delete
...
public class Repository extends org.semanticwb.portal.api.GenericResource
{
    ...
    public void doView(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramsRequest) throws SWBResourceException, IOException {
        ...
        String path1=""+WBUtils.getInstance().getWebPath()+"wbadmin/resources/Repository/images/";
            ...
            else
            {
                ...
                else
                {
                    ...
                    if("view".equals(repfop))
                    {
                        ...
                        if(dir!=null)
                        {
                            if(i_log==1) ret.append(files.getHtml(request, response, user, topic, null,dir,level, paramsRequest));
                        }
                        salir=true;
                    }
                    if(!salir)
        ...
        {
            AFUtils.log(e,"Error in Repository.doView() with resouce id"+": " + base.getId(),true);
        }
...
//            }
        ...
        {AFUtils.log(e,"Error while trying to getWriter() in class Repository.doView()",true);}
    }
    ...
    public void setResourceBase(org.semanticwb.model.Resource base)
    {
        ...
        {
            AFUtils.log(e,"Error on create DocumentBuilder method setResourceBase, resource "+ strRscType +" id "+": "+base.getId(),true);
        }
    }
    ...
    public void install(RecResourceType recobj) throws AFException
    {
        ...
        try
        {
            tmp_conn = (String) com.infotec.appfw.util.AFUtils.getEnv("wb/db/nameconn","wb");
            conn = AFUtils.getDBConnection(tmp_conn,"Repository.install()");
            PreparedStatement pst1  = conn.prepareStatement("select * from wbresourcetype where idtm<>? and objclass=? ");
            pst1.setString(1, recobj.getTopicMapId());
        ...
        if(!existe)
        {
            try
            {
                String dbname=WBUtils.getDBName().toLowerCase();
                if(dbname.lastIndexOf("hsql")>-1) dbname="hsql";
                ...
                InputStream  is_filesql = WBUtils.getInstance().getAdminFileStream("/wbadmin/sql/repository_script_"+dbname+".sql");
                file =AFUtils.getInstance().readInputStream((is_filesql));
                tmp_conn = (String) com.infotec.appfw.util.AFUtils.getEnv("wb/db/nameconn","wb");
                conn = AFUtils.getDBConnection(tmp_conn,"Repository.install() -- !existe --");
                ...
                if(file!=null)
                {
                    ...
                    while(sto.hasMoreTokens())
                    {
                        ...
                        {
                            AFUtils.log(e,"Error on method install of Repository resource trying to execute next code "+": "+ query,true);
                            throw e;
                        }
                    }
                }
            ...
            {
                AFUtils.log(e,"Error on method install Repository resource",true);
            }
            finally
            {
    ...
    public void uninstall(RecResourceType recobj) throws AFException
    {
        ...
        {
            //Connection conn = null;
            String tmp_conn = (String) com.infotec.appfw.util.AFUtils.getEnv("wb/db/nameconn","wb");
            conn = AFUtils.getDBConnection(tmp_conn,"Repository.uninstall()");
        ...
        {
            try{if(rs1 != null) rs1.close();rs1 = null;}catch(Exception e1){}
            try{if(pst1 != null) pst1.close();pst1 = null;}catch(Exception e2){}
            try{if(conn != null) conn.close();conn = null;}catch(Exception e3){}
        }
        ...
        try
        {
            ...
            con = AFUtils.getDBConnection(tmp_conn,"Repository.uninstall() -- repositoy topics -- ");
            pst = con.prepareStatement("select resid, topic from resrepository where idtm=? group by resid, topic");
            pst.setString(1, recobj.getTopicMapId());
            rs = pst.executeQuery();
            while(rs.next())
            {
                ...
                if(strtopic != null)
                {
                    ...
                    Topic topic = tpmgr.getTopicMap(strpartone).getTopic(strTopic);
                    if(topic == null)
                    {
                        TopicMap topicMap = topic.getMap();
        ...
        {
            AFUtils.log(exc,"Error while trying to remove  Repository Topics.",true);
            rs=null;
            pst=null;
            con=null;
        }
        ...
        {
            try{if(rs != null) rs.close();rs = null;}catch(Exception e1){}
            try{if(pst != null) pst.close();pst = null;}catch(Exception e2){}
            try{if(con != null) con.close();con = null;}catch(Exception e3){}
        }
        ...
        if(!existe)
        {
            ...
            {
                //con = WBUtils.getInstance().getDBConnection();
                con = AFUtils.getDBConnection(tmp_conn,"Repository.uninstall() -- drop repository tables -- ");
                ...
                AFUtils.removeDirectory(WBUtils.getInstance().getAppPath()+"/work/"+recobj.getTopicMapId()+"/resources/"+recobj.getName());
                
            }
            catch(Exception e)
            {
                AFUtils.log(e,"Error on method uninstall of Repository resource",true);
            }
            finally
            {
                try{if(rs != null) rs.close();rs = null;}catch(Exception e1){}
                try{if(st != null) st.close();st = null;}catch(Exception e2){}
                try{if(pst != null) pst.close();pst = null;}catch(Exception e3){}
                try{if(con != null) con.close();con = null;}catch(Exception e4){}
            }
        }
        else
        {
            ...
            try
            {
                conn2 = AFUtils.getDBConnection(tmp_conn,"Repository.uninstall() -- delete repository records -- "); //WBUtils.getInstance().getDBConnection();
                pst3  = conn2.prepareStatement("delete from resrepositoryversions where idtm = ?");
                pst3.setString(1, recobj.getTopicMapId());
                pst3.execute();
                pst3.close();
                pst3=null;
                pst2  = conn2.prepareStatement("delete from resrepository where idtm=?");
                pst2.setString(1, recobj.getTopicMapId());
                pst2.executeUpdate();
                pst2.close();
                pst2=null;
                ...
                AFUtils.removeDirectory(WBUtils.getInstance().getAppPath()+"/work/sites/"+recobj.getTopicMapId()+"/resources/"+recobj.getName());
            }
            catch(Exception eb)
            {
                AFUtils.log(eb,"Error while trying to erase repository records. Repository.uninstall",true);
            }
            finally
            {
                try{if(pst2 != null) pst2.close();pst2 = null;}catch(Exception e1){}
                try{if(pst3 != null) pst3.close();pst3 = null;}catch(Exception e2){}
                try{if(conn2 != null) conn2.close();conn2 = null;}catch(Exception e3){}
            }
...
//                topic.getMap().update2DB();
        ...
        if(accion.equals("remove"))
        {
            ...
            try
            {
                ...
                while(rsdocs.next())
                {  // borrando de la tabla de resrepositoryversions
            ...
            {
                AFUtils.log(e,"Error while getting repository file records. Repository.doAdmin(remove)",true);
            }
            finally
            {
                try{if(rsdocs != null) rsdocs.close();rsdocs = null;}catch(Exception e1){}
                try{if(pst != null) pst.close();pst = null;}catch(Exception e2){}
                try{if(conn != null) conn.close();conn = null;}catch(Exception e3){}
            }
            ...
            if(topic!=null)
            {
                ...
                while(itetopics.hasNext())
                {
                    ...
                    {
                        AFUtils.log(e,"Error while trying to erase repository records from the DB. Repository.doAdmin(remove)",true);
                    }
                }
            }
            ...
            if(topic!=null)
            {
                ...
                while(itetopics.hasNext())
                {
                    ...
                    try
                    {
                        conn = AFUtils.getDBConnection(tmp_conn,"Repository.doAdmin() -- remove 2 --");
                        String sql = "delete from resrepository where idtm=? and topic = ?";
                        pst = conn.prepareStatement(sql);
                        pst.setString(1, strTMId);
                        pst.setString(2, tpIter.getId());
                    ...
                    finally
                    {
                        try{if(rsdocs != null) rsdocs.close();rsdocs = null;}catch(Exception e1){}
                        try{if(pst != null) pst.close();pst = null;}catch(Exception e2){}
                        try{if(pst2 != null) pst2.close();pst2 = null;}catch(Exception e3){}
                        try{if(pstlog != null) pstlog.close();pstlog = null;}catch(Exception e4){}
                        try{if(conn != null) conn.close();conn = null;}catch(Exception e5){}
                    }
                }
            }
            ...
            try
            {
                conn = AFUtils.getDBConnection(tmp_conn,"Repository.doAdmin() -- remove 3 --");
            ...
            {
                try{if(psttp != null) psttp.close();psttp = null;}catch(Exception e1){}
                try{if(pstnot != null) pstnot.close();pstnot = null;}catch(Exception e2){}
                try{if(pstlog != null) pstlog.close();pstlog = null;}catch(Exception e3){}
                try{if(conn != null) conn.close();conn = null;}catch(Exception e4){}
            }
        ...
        if(!accion.equals("addrule"))
        {
            if(accion.equals("ShowOld"))
            {
                ...
                {
                    //con = WBUtils.getInstance().getDBConnection();
                    con = AFUtils.getDBConnection(tmp_conn,"Repository.doAdmin() -- recover folder --");
                    ...
                    if(request.getParameter("docid") != null)
                    {
                        ...
                        if(request.getParameter("resdir") != null && request.getParameter("resdir").equals("1"))
                        {
                            ...
                            oTM = TopicMgr.getInstance().getTopicMap(strTMParam);
                            oRep = oTM.getTopic("CNFWB_Rep"+base.getId());
                            oSub = oTM.getTopic(strIdSub,true);
                            ...
                            if(request.getParameter("ifile") != null)
                            {
                                ...
                                for (int i=0; i<values.length; i++)
                                {
                                    ...
                                    if(rsCheck.next())
                                    {
                                        ...
                                        TopicMap tmMain = TopicMgr.getInstance().getTopicMap(tmsid);
                                        tpDir = tmMain.getTopic(tpsid,true);
                                        Topic tpRepo =  tmMain.getTopic("CNFWB_Rep"+base.getId());
                                        if(tpRepo!=null)
                                        {
...
//                                    }
                    ...
                    while(rs.next())
                    {
                        ...
                        {
                            out.println("<a class=link href=\"javascript: DoRecover(" + str_doc + ","+intDeleted+",'"+idSub+"');\">"+"<img src=\""+WBUtils.getInstance().getWebPath()+"wbadmin/images/recover.gif\" border=0 title=\""+paramsRequest.getLocaleString("msgAltRestore")+"\" >"+"</a>");
                        }
                        else
                        {
                            out.println("<img src=\""+WBUtils.getInstance().getWebPath()+"wbadmin/images/recover.gif\" border=0 title=\""+paramsRequest.getLocaleString("msgAltRestore")+"\" >");
                        }
                        out.println("&nbsp;<a class=link href=\"javascript: DoConfirm("+str_doc+");\">"+"<img src=\""+WBUtils.getInstance().getWebPath()+"wbadmin/images/trash_vacio.gif\" border=0 title=\""+paramsRequest.getLocaleString("msgAltEliminate")+"\" >"+"</a>");
                        out.println("</tr>");
                    }
                    con.close();
                    rs.close();
                ...
                {
                    AFUtils.log(e,"Error in method Repository.doAdmin() when action is ShowOld, resource is " + base.getId(),true);
                }
                finally
                {
                    rs = null;
            ...
            else
            {
                ...
                if(strRules.toString().length()>0) strTemp = strRules.toString();
                ...
                if(flag)
                {
                    if(strView.equals("-1")) strView = "0";
                    if(strModify.equals("-1")) strModify = "0";
                    if(strAdmin.equals("-1")) strAdmin = "0";
                    out.println("<script language=javascript>");
        ...
        else
        {
            try
            {
                ...
                if(flag) base.updateAttributesToDB();
            }
            catch(Exception e)
            {AFUtils.log(" Error - Repository.doAdmin.addrule");}
    ...
    public boolean canRecover(String p_tm, String p_topic)
    {
        ...
        s_topicmap = p_tm;
        s_topic = p_topic;
        TopicMap topicMap = tpmgr.getTopicMap(s_topicmap);
        Topic topic = topicMap.getTopic(s_topic,true);
        if(topic != null)
        {
            result = true;
        }
     ...
     * @param Repository Topic object that represents the main directory of the repository
     */
    public void recuperarFolder(Topic tp, Topic Repository)
    {
        try
        {
            tp.getDbdata().setDeleted(0);
            tp.getDbdata().update();
            if(tp.getType()!=null && !Repository.getId().equals(tp.getType().getId()) && tp.getType().getDbdata().getDeleted()==1)
            {
                recuperarFolder(tp.getType(), Repository);
            }
        }
        catch(Exception e)
        {AFUtils.log(e,"Error al recuperar los sub directorios eliminados.",true);}
    }
     ...
     */
    public void checkDir(TopicMap tm, Topic dir, Topic repository, long id)
    {
        Connection conn = null;
        PreparedStatement pst = null;
        ResultSet rs = null;
        if(dir!=null && repository!=null && !dir.getId().equals(repository.getId()))
        {
            ...
            {
                String tmp_conn = (String) com.infotec.appfw.util.AFUtils.getEnv("wb/db/nameconn","wb");
                conn = AFUtils.getDBConnection(tmp_conn,"Repository.checkDir()");
                //conn=WBUtils.getInstance().getDBConnection();
                ...
                {
                    Topic tptmp = dir.getType();
                    tm.removeTopic(dir.getId());
                    tm.update2DB();
                    rs.close();
                    pst.close();
                    conn.close();
                    if(tptmp!=null && !repository.getId().equals(tptmp.getId()) && tptmp.getDbdata().getDeleted()==1)
                    {
            ...
            {AFUtils.log(e,"Error al eliminar los sub folders.",true);}
            finally
            {
                try{if(rs != null) rs.close();rs = null;}catch(Exception e1){}
                try{if(pst != null) pst.close();pst = null;}catch(Exception e2){}
     ...
     */
    public String tmApplet(String tpid, String tmid, WBParamRequest paramsRequest)
    {
        
        StringBuffer sbfRet = new StringBuffer();
        sbfRet.append("<APPLET id=\"appttmadmin\" name=\"appttmadmin\" code=\"applets.mapsadm.TMWBAdmin.class\" codebase=\""+WBUtils.getInstance().getWebPath()+"\" ARCHIVE=\"wbadmin/lib/TMWBAdmin.jar, wbadmin/lib/WBCommons.jar\" width=\"100%\" height=\"100%\">");
...
//        String idtm=base.getTopicMapId();
...
//        catch(Exception e)
//        {
//            AFUtils.log(e,WBUtils.dateFormat(new Timestamp(System.currentTimeMillis()))+" - Error while trying to index files of Repository resource with id:"+resid+", from TopicMap:"+idtm,false);
//        }
//    }
}
