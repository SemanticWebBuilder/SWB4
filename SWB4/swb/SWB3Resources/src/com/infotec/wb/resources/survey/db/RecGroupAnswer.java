 ...
 * 
 */
public class RecGroupAnswer {

    private static Logger log = SWBUtils.getLogger(RecGroupAnswer.class);
    ...
    public void load(){
        ...
        try{
            ...
            pst = conn.prepareStatement("select * from sr_groupanswer where groupaid = ? and idtm=?");
            pst.setInt(1,groupaid);
            ...
            log.error("Error while trying to load a record from sr_groupanswer",e);
        }
        finally{
            rs=null;
            pst=null;
    ...
    public boolean create(){
        ...
        if(groupaid == 0){
            ...
            try{
                ...
                pst = conn.prepareStatement("select max(groupaid) from sr_groupanswer where idtm=?");
                pst.setString(1,idtm);
                rs = pst.executeQuery();
                if(rs.next()){
                    groupaid = rs.getInt(1) + 1;
                }
                else{
                    groupaid = 1;
                }
                ...
                s_query = "insert into sr_groupanswer (groupaid, idtm, title, description, lastupdate) values (?, ?, ?, ?, ?)";
                pst = conn.prepareStatement(s_query);
                pst.setLong(1, groupaid);
                pst.setString(2,idtm);
                ...
                log.error("Error while trying to create a record on sr_groupanswer",e);
            }
            finally{
                rs=null;
                pst=null;
    ...
    public boolean update(){
        ...
        try{
            conn = SWBUtils.DB.getDefaultConnection();
            lastupdate = new Timestamp(new java.util.Date().getTime());
            s_query = "update sr_groupanswer set title=?, description=?, lastupdate=? where groupaid=? and idtm=?";
            pst = conn.prepareStatement(s_query);
            pst.setString(1, title);
            pst.setString(2, description);
            pst.setTimestamp(3, lastupdate);
            pst.setInt(4, groupaid);
            ...
            log.error("Error while trying to update a record on sr_groupanswer",e);
        }
        finally{
            pst=null;
            conn=null;
    ...
    public boolean remove(){
        ...
        try{
            conn = SWBUtils.DB.getDefaultConnection();
            s_query = "delete from sr_groupanswer where groupaid=? and idtm=?";
            pst = conn.prepareStatement(s_query);
            pst.setInt(1, groupaid);
            pst.setString(2,idtm);
            pst.executeUpdate();
            ...
            log.error("Error while trying to remove a record from sr_groupanswer",e);
        }
        finally{
            pst=null;
            conn=null;
