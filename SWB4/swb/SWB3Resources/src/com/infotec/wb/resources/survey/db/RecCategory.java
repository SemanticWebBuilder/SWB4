 ...
 *
 * Created Juan Antonio Fernández Arias
 * 
 */
public class RecCategory {

    private static Logger log = SWBUtils.getLogger(RecCategory.class);
     ...
     * @param categoryid input int value
     */    
    ...
    public void load(){
        ...
        try{
            ...
            pst = conn.prepareStatement("select * from sr_category where categoryid = ? and idtm=?");
            ...
            log.error("Error while trying to load a record from sr_category",e);
        }
        finally{
            rs=null;
            pst=null;
    ...
    public boolean create(){
        ...
        if(categoryid == 0){
            ...
            try{
                ...
                pst = conn.prepareStatement("select max(categoryid) from sr_category where idtm=?");
                ...
                s_query = "insert into sr_category (categoryid, idtm, title, description, lastupdate) values (?, ?, ?, ?, ?)";
                ...
                log.error("Error while trying to create a record on sr_category",e);
            }
            finally{
                rs=null;
                pst=null;
    ...
    public boolean update(){
        ...
        try{
            conn = SWBUtils.DB.getDefaultConnection();
            lastupdate = new Timestamp(new java.util.Date().getTime());
            s_query = "update sr_category set title=?, description=?, lastupdate=? where categoryid=? and idtm=?";
            pst = conn.prepareStatement(s_query);
            ...
            log.error("Error while trying to update a record on sr_category",e);
        }
        finally{
            pst=null;
            conn=null;
    ...
    public boolean remove(){
        ...
        try{
            conn = SWBUtils.DB.getDefaultConnection();
            s_query = "delete from sr_category where categoryid=? and idtm=?";
            pst = conn.prepareStatement(s_query);
            pst.setInt(1, categoryid);
            pst.setString(2,idtm);
            pst.executeUpdate();
            ...
            log.error("Error while trying to remove a record from sr_category",e);
        }
        finally{
            pst=null;
            conn=null;
